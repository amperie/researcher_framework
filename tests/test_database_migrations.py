...
def test_new_validates_names_and_versions(versions):
    assert new("add_index", versions).name == "003_add_index.sql"
    with pytest.raises(ValueError):
        new("../bad", versions)
    (versions / "003_duplicate.sql").write_text("SELECT 1;")
...
def test_commit_cannot_leave_partial_schema(postgres, versions):
    (versions / "003_partial.sql").write_text(
...
def test_pending_apply_and_concurrent_runners(postgres, versions):
    (versions / "003_marker.sql").write_text("CREATE TABLE researcher.migration_marker (id integer);")
    ...
    assert sorted(results, key=len) == [[], ["003_marker.sql"]]
...
def test_failure_rolls_back_schema_and_history(postgres, versions):
    (versions / "003_marker.sql").write_text("CREATE TABLE researcher.migration_marker (id integer);")
    (versions / "004_broken.sql").write_text("SELECT nonexistent_migration_function();")
